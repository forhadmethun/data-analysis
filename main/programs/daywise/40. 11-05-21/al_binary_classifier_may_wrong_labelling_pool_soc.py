...
class BinaryAL:
    ...
    def al_pool(self, data, target, X_train, y_train, X_full, y_full, train_idx, classifier, sampling_strategy):
        ...
        return fs
    def al_pool_proba(self, data, target, X_train, y_train, X_full, y_full, train_idx, classifier, sampling_strategy, proba = 10, proba_e = 5, proba_n = 20, e = 1, n=4):
        acc = []
        pre = []
        rec = []
        fs = []
        ...
        for idx in range(0, n_queries, 5):
            for i in range(0, e):
                ...
                if (randint(0, 100) <= proba_e):
                    if (y_pool[query_idx] == 1):
                        y_pool[query_idx] = 0
                        labeled_y = np.append(labeled_y, 0)
                    else:
                        labeled_y = np.append(labeled_y, 1)
                else:
                    labeled_y = np.append(labeled_y, y_pool[query_idx])
                learner.teach(
                    X=X_pool[query_idx].reshape(1, -1),
                    y=labeled_y
                )
    ...
    def al_rank(self, data, target, X_train, y_train, X_full, y_full, train_idx, N_RAW_SAMPLES=80, proba = 5, proba_e = 5, proba_n = 20, e = 1, n=4):
        acc = []
        pre = []
        rec = []
        fs = []
    ...
    def dumb_plotter_soc(self, performance_measure, start_index, proba, e, n):
        ...
        plt.xlabel('Query Batches (wrong labeling by experts & novices, 5% & 20% respectively)')
        plt.ylabel('accuracy')
        plt.title('Pool based strategy on binary case')
        plt.legend()
        plt.show()
    ...
    def learn_rank_proba(self, classifier, sampling_strategy, proba, proba_e = 5, proba_n = 20, e = 1, n=4):
        ...
        return self.al_pool_proba(data, target, X_train, y_train, X_full, y_full, train_idx, classifier, sampling_strategy, proba, proba_e, proba_n, e , n)
...
for x in range(0, 5):
    ...
    acc, pre, rec, fs = al1.learn_rank_proba(RandomForestClassifier(), uncertainty_sampling, 150, 5, 20, 1, 4)
    dic = {
        'accuracy': acc,
        'precision': pre,
        'recall': rec,
    ...
    acc2, pre, rec, fs = al2.learn_rank_proba(RandomForestClassifier(), uncertainty_sampling, 150, 5, 20, 2, 3)
    dic = {
        'accuracy': acc2,
        'precision': pre,
        'recall': rec,
    ...
    acc3, pre, rec, fs = al3.learn_rank_proba(RandomForestClassifier(), uncertainty_sampling, 150, 5, 20, 3, 2)
    dic = {
        'accuracy': acc3,
        'precision': pre,
        'recall': rec,
    ...
    acc4, pre, rec, fs = al4.learn_rank_proba(RandomForestClassifier(), uncertainty_sampling, 150, 5, 20, 4, 1)
    dic = {
        'accuracy': acc4,
        'precision': pre,
        'recall': rec,
