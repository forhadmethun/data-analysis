...
def applyModels(X, y, n):
    ...
    for name, model in models:
        ...
        mean_cross_val_score = cross_val.mean()
        # score = clf.score(y_test, y_pred)
        # print("DT model score: " + str(score))
        print("DT cross val score: " + str(mean_cross_val_score))
...
def normalize_datagram(df, n):
    ...
    removal_list = []
    for index, val in enumerate(header_feature):
        if header_attack_cat[index] == 'Normal' and (category_item_list['Normal']['count'] > 9000):
            removal_list.append(index)
            continue

        elif header_attack_cat[index] != 'Normal' and (category_item_list[header_attack_cat[index]]['count'] > n):
            removal_list.append(index)
            continue
