...
print("[%20s, %10s, %10s]" % ('feature', 'gini index', 'entropy'))
for feature in df.drop(columns = 'id').columns:
    ...
    # print('[' + feature + ',' + str(compute_impurity(df[feature], IMPURITY_TYPE.GINI_INDEX)) + ',' +  str(compute_impurity(df[feature], IMPURITY_TYPE.ENTROYPY))+ ']')
    print('[%20s, %10s, %10s]' % ( feature , str(compute_impurity(df[feature], IMPURITY_TYPE.GINI_INDEX)), str(compute_impurity(df[feature], IMPURITY_TYPE.ENTROYPY))))
