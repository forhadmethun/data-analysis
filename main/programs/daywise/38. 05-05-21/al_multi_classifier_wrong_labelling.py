...
class BinaryAL:
    ...
    def al_pool_proba(self, data, target, X_train, y_train, X_full, y_full, train_idx, classifier, sampling_strategy, proba):
        ...
        for idx in range(n_queries):
            ...
            if(rand_int <= proba):
                labeled_y = np.array((randint(0,9))).reshape(1, )
    ...
    def plotter(self, y1, y2, y3, y4, how_many_max_instances, start_index):
        ...
        plt.ylabel('F1 Score')
        plt.title('Pool based Active learning accuracy performance on multiclass case')
        plt.legend()
        plt.show()
    ...
    def dumb_plotter_proba(self, performance_measure, start_index, proba):
        ...
        plt.title('Pool based selection strategy on multiclass case(' + str(proba) + '% wrong labeling' + ')')
        plt.legend()
        plt.show()
