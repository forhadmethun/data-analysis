...
class BinaryAL:
    ...
    def normalize_datagram(self, df, n):
        ...
        header_feature = df[feature].tolist()
        header_attack_cat = df[feature].tolist()
        # header_attack_cat = df['attack_cat'].tolist()
        for index, val in enumerate(header_feature):
    ...
    def compute_fisher_sorted(self, df):
        ...
        for feature in df.drop(columns=['id', 'proto', 'service', 'state', 'attack_cat', 'label']).columns:
            header_attack_cat = df['attack_cat'].tolist()  # this is for binary class classifier
    ...
    def al_rank(self, data, target, X_train, y_train, X_full, y_full, train_idx,  N_RAW_SAMPLES = 80, batch_size = 3):
        ...
        for index in range(N_QUERIES):
            ...
            accuracy, precision, recall, fscore = self.performance_measure(learner, X_full, y_full)
    ...
    def dumb_plotter_rank(self, performance_measure, start_index, batch_size):
        ...
        plt.title('Rank based Active learning performance on multiclass case')
        plt.legend()
        plt.show()
...
# al1.learnAndPlot()
al1.learnAndPlotRankBased(1700, 3)


al2 = BinaryAL(20, 1700)
# al1.learnAndPlot()
al2.learnAndPlotRankBased(1700, 5)


al3 = BinaryAL(20, 1700)
# al1.learnAndPlot()
al3.learnAndPlotRankBased(1700, 7)


al4 = BinaryAL(20, 1700)
# al1.learnAndPlot()
al4.learnAndPlotRankBased(1700, 9)
